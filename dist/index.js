...
var BeeProxy = /*#__PURE__*/function () {
  function BeeProxy() {
  ...
  _createClass(BeeProxy, [{
    ...
    value: function _checkConfigFileExist() {
      if (!fs.existsSync(this.configFilePath)) {
        log.error("No wproxy.js folder found in ".concat(this.rootPath));
        process.exit(-1);
      }
    }
  }, {
    ...
    value: function _getCurrentEnvAndRuleList() {
      ...
      if (!currentEnv) {
        log.warn("You have not specify backend env,use ".concat(envList[0], " as default"));
    ...
    value: function _executeDirResponder(mockPath, req, res) {
      ...
      var jsonPath = path.join(this.rootPath, mockPath, "".concat(url, ".json"));
    ...
    value: function executeResponderProcessor(responderProcessor, req, res) {
      ...
      if (!responderType) {
        log.error("".concat(responderType, " config error, please retry after fix it"));
        process.exit(-1);
      }
